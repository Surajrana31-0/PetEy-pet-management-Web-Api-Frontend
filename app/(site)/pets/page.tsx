    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8">
        ...
        <p className="mt-1 text-muted-foreground">
          {pets ? `${filtered.length} of ${pets.length} pets` : 'Loading pets…'}
        </p>
      </div>
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div className="flex items-center gap-2">
          <Select value={speciesFilter} onValueChange={(v) => setSpeciesFilter(v as PetSpecies | 'ALL')}>
            <SelectTrigger className="w-[130px]" aria-label="Filter by species">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
          ...
          <Select value={sortBy} onValueChange={(v) => setSortBy(v as SortOption)}>
            <SelectTrigger className="w-[130px]" aria-label="Sort pets">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
          ...
          <div className="hidden items-center rounded-lg border border-border sm:flex">
            <Button
              ...
              size="icon"
              className={cn('rounded-r-none', viewMode === 'grid' && 'gradient-warm text-white')}
            ...
            <Button
              ...
              size="icon"
              className={cn('rounded-l-none', viewMode === 'list' && 'gradient-warm text-white')}
      ...
      {hasActiveFilters && (
        <div className="mb-4 flex items-center gap-2">
          <Badge variant="secondary" className="gap-1">
            <SlidersHorizontal className="h-3 w-3" /> Filters active
          </Badge>
      ...
      {!isLoading && !error && filtered.length === 0 && (
        <EmptyState
          icon={PawPrint}
          title={hasActiveFilters ? 'No pets match your filters' : 'No pets available yet'}
          description={
            hasActiveFilters
        ...
        <>
          {viewMode === 'grid' ? (
            ...
            <div className="flex flex-col gap-3">
              {filtered.map((pet) => (
                <PetCard key={pet._id} pet={pet} className="flex-row" />
              ))}
            </div>
          )}
        </>
