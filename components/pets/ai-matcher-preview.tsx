...
const DEMO_MATCHES: IAiPetMatch[] = [
  {
    ...
    score: 92,
    reason: 'Great match for active families who enjoy outdoor adventures.',
  },
  {
    pet: {
      _id: 'demo-2',
    ...
    score: 87,
    reason: 'Ideal for apartment living with a relaxed daily routine.',
  },
];
...
export function AiMatcherPreview() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (!result.success) {
        setError(result.message ?? 'AI service unavailable — showing sample matches.');
      }
    });
  };
  ...
  return (
    <div className="ai-matcher">
      <div className="ai-matcher-grid">
        <AnimateIn>
          <form onSubmit={handleSubmit} className="ai-matcher-form">
            <h2 className="ai-matcher-form-title">Tell us about your lifestyle</h2>
            <p className="ai-matcher-form-desc">
              Our AI analyzes your preferences to recommend pets with the best compatibility.
            </p>

            <div className="ai-form-field">
              <label htmlFor="lifestyle">Living situation</label>
              <Select
                id="lifestyle"
                value={form.lifestyle}
                onChange={(e) => setForm((f) => ({ ...f, lifestyle: e.target.value }))}
              >
                <option value="active">Active & outdoorsy</option>
                <option value="moderate">Balanced lifestyle</option>
                <option value="calm">Calm & relaxed</option>
              </Select>
            </div>

            <div className="ai-form-field">
              <label htmlFor="housingType">Home type</label>
              <Select
                id="housingType"
            ...
            <div className="ai-form-field">
              ...
              >
                <option value="low">Low — prefer quiet companions</option>
                <option value="moderate">Moderate — daily walks & play</option>
                <option value="high">High — runs, hikes, adventures</option>
              </Select>
            </div>
            ...
            <div className="ai-form-checkboxes">
              ...
              <label className="ai-checkbox">
                <input
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, hasOtherPets: e.target.checked }))}
                />
                I have other pets
              </label>
            </div>
            ...
            <div className="ai-form-field">
              ...
              <Textarea
                ...
                placeholder="e.g. Prefer a hypoallergenic breed, good with kids, low shedding..."
                rows={4}
              />
            </div>
        ...
        <div className="ai-matcher-results">
          {matches === null ? (
            <AnimateIn delay={100}>
              <div className="ai-matcher-empty">
                <div className="ai-matcher-empty-icon">
                  <Sparkles className="h-8 w-8" aria-hidden />
                </div>
                <h3>Your AI matches will appear here</h3>
                <p>
                  Fill in your lifestyle preferences and let our AI find companions
                  that fit your home, routine, and personality.
                </p>
              </div>
            </AnimateIn>
          ) : (
            <>
              {error && (
                <p className="ai-matcher-notice" role="status">
                  {error}
                </p>
              )}
              {usedDemo && !error && (
                <p className="ai-matcher-notice" role="status">
                  Showing sample matches — sign in for personalized AI recommendations.
                </p>
              )}
                ...
                {matches.map((match, index) => (
                  <AnimateIn key={match.pet._id} delay={index * 80}>
                    <article className="ai-match-card">
                      <div className="ai-match-image">
                        <SafeImage
                          src={getPetImage(match.pet, index)}
                          alt={match.pet.name}
                          width={120}
                          height={120}
                      ...
                      <div className="ai-match-body">
                        <div className="ai-match-header">
                          <div>
                            ...
                            <p>
                              {match.pet.breed} · {PET_SPECIES_LABELS[match.pet.species]}
                            </p>
                          </div>
                          <Badge variant="success">{match.score}% match</Badge>
                        </div>
                        <p className="ai-match-reason">{match.reason}</p>
                        {match.pet._id.startsWith('demo') ? (
                          <Link href="/adopt" className="ai-match-link">
                            Browse all pets →
                          </Link>
                        ) : (
                          <Link href={`/pets/${match.pet._id}`} className="ai-match-link">
                            View profile →
                          </Link>
                        )}
                      </div>
              ...
              <div className="ai-matcher-cta">
                <p>Want full AI recommendations and chat?</p>
